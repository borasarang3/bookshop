...
public class FileService {
    ...
    // 컨트롤러에서 받은 MultipartFile을 byte 타입으로 입력받음

    public String uploadFile(String uploadPath, String originalFileName,
                             byte[] fileDate) throws IOException {

        UUID uuid = UUID.randomUUID();
        log.info("생성된 uuid : " + uuid);
        //확장자 // abcd.jpg 자르다 substring ( abcd.jpg 중에서 "." 구분자를 기준으로 )
        String extension = originalFileName.substring(originalFileName.indexOf("."));
        log.info("파일명 . 확장자 제외 : " + extension);

        // asdsafsf12314 + abcd // 중복저장 방지
        String savedFileName = uuid.toString() + extension;

        // 전체 경로 /shop/item/asdsfa + abcd
